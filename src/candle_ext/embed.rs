...
// Output functions
fn array_float_output(_: &[Field]) -> PolarsResult<Field> {
	Ok(Field::new("array_float", DataType::List(Box::new(DataType::Float32))))
}
...
#[polars_expr(output_type_func=array_float_output)]
pub fn embed_text(
	s: &[Series],
    ...
    let (some_sentences_idx, none_sentences_idx): (Vec<_>, Vec<_>) = sentences
        .iter()
        .enumerate()
        .partition(|(_, sentence)| sentence.is_some());

    // Handle Some sentences:
    let some_sentences: Vec<&str> = some_sentences_idx.iter().filter_map(|(_, sentence)| **sentence).collect();
    let embeddings = model.encode_batch(some_sentences, false).map_err(
        |e| polars_err!(ComputeError: "Encoding failed with error:\n{}", e)
    )?;

    let (_, emb_dim) = embeddings.dims2()
        .map_err(|e| polars_err!(ComputeError: "Getting embeddings dimensions failed with error:\n{}", e))?;
    ...
    some_sentences_idx.into_iter().zip(emb_arr.into_iter()).for_each(|((idx, _), emb)| {
        arr[idx] = Some(emb);
    });

    // Handle None sentences:
    for (idx, _) in none_sentences_idx {
        arr[idx] = None;
    }

    // TODO: How to directly create a array of fixed size list arrays?
    let mut builder = ListPrimitiveChunkedBuilder
        ::<Float32Type>::new(&format!("{}_embedding", name), len, 8, DataType::Float32);
    for embedding_option in arr {
